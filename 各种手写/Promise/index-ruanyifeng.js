...
// const fetch  = (arg, delay) => new Promise((resolve, reject) => {
...
// fetch如果率先返回结果了就直接把返回值传给p的then内回调，否则就要执行后面的Promise把状态reject
...
// //   return Promise.allSettled([resolved, rejected]);
// // }

// async function change(rejected, resolved) {
//   const res = await Promise.allSettled([resolved, rejected])
//   const successfulPromises = res.filter(i => i.status === 'fulfilled')
//   console.log(successfulPromises);
// }

// change(rejected, resolved)
